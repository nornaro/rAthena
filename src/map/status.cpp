...
#ifdef RENEWAL_DB // Renewal formulas
	if (bl->type == BL_HOM) {
		// Def2
...
// ----- HIT CALCULATION -----
	...
	if (((skill = pc_checkskill(sd, NJ_SYURIKEN)) > 0) && sd->inventory_data[sd->equip_index[EQI_AMMO]] && sd->inventory_data[sd->equip_index[EQI_AMMO]]->look == A_SHURIKEN)
		base_status->rhw.range += 8;
