...
static int battle_get_weapon_element(struct Damage* wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, short weapon_position, bool calc_for_damage_only)
{
	...
	switch( skill_id ) {
		...
		case AS_VENOMKNIFE:
			if (sd && sd->flicker)
				element = battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_R, false);
			break;
	}
...
static void battle_calc_attack_masteries(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	{	//Add mastery damage
		...
		if (skill_id == NJ_SYURIKEN && (skill = pc_checkskill(sd,NJ_TOBIDOUGU)) > 0) { // !TODO: Confirm new mastery formula
			//ATK_ADD(wd->damage, wd->damage2, wd->damage*2+wd->damage2*2+3 * skill+4 * pc_checkskill(sd, NJ_SYURIKEN));
			ATK_ADD(wd->damage, wd->damage2, sstatus->str * 2 + 3 * skill + 4 * pc_checkskill(sd, NJ_SYURIKEN) + sd->inventory_data[sd->equip_index[EQI_AMMO]]->atk);
#ifdef RENEWAL
	...
	switch (skill_id) {	//Calc base damage according to skill
		...
		default:
			...
			if (sd)
			{
				...
				if (sc && sc->data[SC_EDP] && (skill_id == GC_DARKILLUSION || skill_id == GC_WEAPONCRUSH ||
					skill_id == GC_VENOMPRESSURE || skill_id == GC_PHANTOMMENACE || skill_id == GC_ROLLINGCUTTER ||
					skill_id == GC_CROSSRIPPERSLASHER))
				{
					...
					{
						ATK_ADDRATE(wd->damage, wd->damage2, edpva + edpvb * sc->data[SC_EDP]->val1);
					}
					else
					{
						ATK_ADDRATE(wd->damage, wd->damage2, edpva - edpvb * sc->data[SC_EDP]->val1);
				...
				if (sd->status.party_id && (pc_checkskill(sd, TK_POWER)))
				{
					if ((i = party_foreachsamemap(party_sub_count, sd, 0)) > 1) // exclude the player himself [Inkfish]
						ATK_ADDRATE(wd->damage, wd->damage2, 2 * pc_checkskill(sd, TK_POWER) * i);
				}
			}
			break;
...
//			skillratio += -100 + 200 * skill_lv;
			if (sd && pc_checkskill(sd, AM_LEARNINGPOTION))
				skillratio += 20 * pc_checkskill(sd, AM_LEARNINGPOTION);
//#else
			if (status_get_class_(target) == CLASS_BOSS) {
				skillratio += -100 + 200 * skill_lv;
			} else {
...
#endif
			if(sd)
				skillratio += pc_checkskill(sd,AS_POISONREACT) * battle_config.splasher_react;
			break;
#ifndef RENEWAL
...
//#endif
			if (sc){
				if (sc->data[SC_BUNSINJYUTSU] && (i = sc->data[SC_BUNSINJYUTSU]->val2) > 0) // mirror image bonus only occurs if active
					skillratio += i * 100;
				if (sc->data[SC_NEN])
					skillratio += skillratio / 5 * pc_checkskill(sd, NJ_NEN);
			}
			break;
		case NJ_KIRIKAGE:
//#ifdef RENEWAL
...
//#ifdef RENEWAL
	...
	switch (skill_id) {
		...
		//case NC_MAGMA_ERUPTION:
			...
			break;
		/*case NJ_SYURIKEN:
			wd.damage = 0;//battle_attr_fix(src, target, wd.damage, ELE_NEUTRAL, tstatus->def_ele, tstatus->ele_lv);
			break;*/
		case CR_SHIELDBOOMERANG:
		case LK_SPIRALPIERCE:
		case ML_SPIRALPIERCE:
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		if(!flag.imdef){
			 ...
			 */
			//if (target->type != BL_PC) {
				mdef = mdef / battle_config.mdef_adjust_rate + (100 - mdef) / 10;
				if (mdef < 0) mdef = 0;
			//}
#else
			if (target->type != BL_PC) {
				mdef = mdef * battle_config.mdef_adjust_rate;
				if (mdef < 0) mdef = 0;
...
//#ifdef RENEWAL
		 case NJ_ISSEN:
			...
			// modified def formula
			{
				...
				//md.damage = 40 * atk.damage + (int64)sstatus->hp + (atk.damage * (int64)sstatus->hp / (int64)sstatus->max_hp * skill_lv);
				md.damage = (int64)sstatus->hp * skill_lv / 10 + 40 * atk.damage;
...
*/
		sd->state.arrow_atk = (sd->status.weapon == W_BOW || (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE) || (sd->inventory_data[sd->equip_index[EQI_AMMO]] && sd->inventory_data[index]->look == A_SHURIKEN));
	...
	if (sd && (skillv = pc_checkskill(sd, NJ_SYURIKEN)) > 0 && sd->inventory_data[sd->equip_index[EQI_AMMO]] && sd->inventory_data[sd->equip_index[EQI_AMMO]]->look == A_SHURIKEN) {
...
static const struct _battle_data {
	...
	{ "item_auto_store",                    &battle_config.item_auto_store,                 10000,  0,      10000,          },
...
// rAthena
	...
	{ "disable_lvl_restriction",            &battle_config.disable_lvl_restriction,          0,      0,      MAX_LEVEL,              },
	{ "def_adjust_rate",                    &battle_config.def_adjust_rate,                 10,     0,      CHAR_MAX,       },
	{ "mdef_adjust_rate",                   &battle_config.mdef_adjust_rate,                5,      0,      CHAR_MAX,       },
