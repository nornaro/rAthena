...
uint8 pc_isequip(struct map_session_data *sd,int n)
{
	...
	if (battle_config.enable_lvl_restriction) {
		if (item->elv <= battle_config.disable_lvl_restriction ||
			sd->status.base_level <= battle_config.disable_lvl_restriction) {
...
bool pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	if( (DIFF_TICK(sd->canuseitem_tick, gettick()) > 0) && (sd->inventory_data[n]->type == IT_HEALING) ||
