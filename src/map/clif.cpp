...
#ifdef RENEWAL
	WBUFW(buf,43) = status->def * battle_config.def_adjust_rate + status->def2;
	WBUFW(buf,45) = status->mdef * battle_config.mdef_adjust_rate + status->mdef2;
#else
	WBUFW(buf,43) = status->def / battle_config.def_adjust_rate + status->vit;
	WBUFW(buf,45) = status->mdef / battle_config.mdef_adjust_rate + (100 - status->mdef) / 10;
#endif
...
void clif_updatestatus(struct map_session_data *sd,int type)
{
	...
	case SP_DEF2:
		...
		break;
	case SP_MDEF2:
		WFIFOL(fd, 4) = (pc_rightside_mdef(sd) < 0) ? 0 : pc_rightside_mdef(sd);
		break;
	case SP_CRITICAL:
...
#ifdef RENEWAL
	WBUFW(buf,24) = pc_leftside_def(sd) * battle_config.def_adjust_rate;
	WBUFW(buf,28) = pc_leftside_mdef(sd)/* * battle_config.mdef_adjust_rate*/;
#else
	WBUFW(buf, 24) = pc_leftside_def(sd) / battle_config.def_adjust_rate;
	WBUFW(buf, 28) = pc_leftside_mdef(sd)/* / battle_config.mdef_adjust_rate*/;
#endif
...
int clif_damage(struct block_list* src, struct block_list* dst, t_tick tick, int sdelay, int ddelay, int64 sdamage, int div, enum e_damage_type type, int64 sdamage2, bool spdamage)
{
	...
	if (type != DMG_MULTI_HIT_CRITICAL)
		type = clif_calc_delay(type,div,damage+damage2,ddelay);	sc = status_get_sc(dst);
	if(sc && sc->count) {
...
void clif_skill_estimation(struct map_session_data *sd,struct block_list *dst)
{
	...
	int mdef = battle_config.estimation_type & 1 ? status->mdef : 0;
#ifdef RENEWAL
	WBUFW(buf,12)= (battle_config.estimation_type&1?status->def:0) * battle_config.def_adjust_rate
		+(battle_config.estimation_type&2?status->def2:0);
	WBUFW(buf,14)=status->race;
	WBUFW(buf,16)= (battle_config.estimation_type&1?status->mdef:0) * battle_config.mdef_adjust_rate
		+(battle_config.estimation_type&2?status->mdef2:0);
#else
	WBUFW(buf, 12) = (battle_config.estimation_type & 1 ? status->def : 0) / battle_config.def_adjust_rate
		+ (battle_config.estimation_type & 2 ? status->def2 : 0);
	WBUFW(buf, 14) = status->race;
	WBUFW(buf, 16) = mdef / battle_config.mdef_adjust_rate + (100 - mdef) / 10
		+ (battle_config.estimation_type & 2 ? status->mdef2 : 0);
#endif
...
void clif_check(int fd, struct map_session_data* pl_sd)
{
	...
	WFIFOW(fd,22) = pl_sd->battle_status.def / battle_config.def_adjust_rate;
...
#ifdef RENEWAL
		case SP_DEF1:
			WFIFOL(fd,4) = status->def * battle_config.def_adjust_rate;
			break;
		case SP_MDEF1:
			WFIFOL(fd,4) = status->mdef * battle_config.mdef_adjust_rate;
			break;
#else
		case SP_DEF1:
			WFIFOL(fd, 4) = status->def / battle_config.def_adjust_rate;
			break;
		case SP_MDEF1:
			WFIFOL(fd, 4) = status->mdef / battle_config.mdef_adjust_rate + (100 - status->mdef) / 10;
			break;
#endif
		case SP_MERCFLEE:
...
#ifdef RENEWAL
	WFIFOW(fd, 14) = status->def * battle_config.def_adjust_rate + status->def2;
	WFIFOW(fd, 16) = status->mdef * battle_config.mdef_adjust_rate + status->mdef2;
#else
	WFIFOW(fd, 14) = status->def / battle_config.def_adjust_rate + status->vit;
	WFIFOW(fd, 16) = status->mdef / battle_config.mdef_adjust_rate;
#endif
